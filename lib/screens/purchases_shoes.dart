...
import 'package:shoes_app_ui_training/widgets/purchased_shoe_widget.dart';

class PurchaseShoes extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return isOrdered
        ? ref.watch(getShoesInPurchasesCostDescendingProvider(user.uid)).when(
              data: (data) {
                return ListView.builder(
                    itemCount: data.length,
                    itemBuilder: (BuildContext context, int index) {
                      final cart = data[index];
                      return PurchasedShoe(cart: cart);
                    });
              },
        ...
        : ref.watch(getShoesInPurchasesProvider(user.uid)).when(
              data: (data) {
                return ListView.builder(
                    itemCount: data.length,
                    itemBuilder: (BuildContext context, int index) {
